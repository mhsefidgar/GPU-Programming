...
if isempty(otherData)
    allDataFiles = dir( fullfile( gpubench.getDataDir(), 'R*.mat' ) );
...
if exist( datafilename, 'file' ) == 2
    ...
    if ~isempty(existingData)
        ...
        existingNames = cell(numel(existingData), 1);
        for ii=1:numel(existingData)
            if existingData(ii).IsHostData
                existingNames{ii} = existingData(ii).CPUInfo.Name;
            else
                existingNames{ii} = existingData(ii).GPUInfo.Name;
            end
        end
        % Extract the names for comparison
        otherNames = cell(numel(otherData), 1);
        for ii=1:numel(otherData)
            if otherData(ii).IsHostData
                otherNames{ii} = otherData(ii).CPUInfo.Name;
            else
                otherNames{ii} = otherData(ii).GPUInfo.Name;
            end
        end
