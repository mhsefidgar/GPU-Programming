...
%   if it already exists updates the value. LABEL must be a character
%   vector and VALUE a number between zero and one or the character vector
%   'Close' to remove the entry Setting value equal to 0 or 'Reset' will
%   cause the progress bar to reset and the time estimate to be
%   re-initialized.
%
...
narginchk(1,inf);
if ~ischar( label )
    error( 'multiWaitbar:BadArg', 'LABEL must be the name of the progress entry' );
end
    ...
    for ii=1:numel( params )
        switch upper( params{ii} )
            ...
            case {'RELABEL', 'UPDATELABEL'}
                ...
                if ~ischar( values{ii} )
                    error( 'multiWaitbar:BadString', 'Value for ''Relabel'' must be a character vector.' );
                end
